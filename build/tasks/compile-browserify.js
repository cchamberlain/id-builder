...
var fs, browserify, lsr, async, log, debug, info, warning, ref$, map, filter, file, sourceExtension, targetExtension, sourceFilePathMatches, compileFile, compileAllFiles, out$ = typeof exports != 'undefined' && exports || this;
fs = require('fs');
...
out$.sourceFilePathMatches = sourceFilePathMatches = curry$(function(options, task, sourceFilePath){
  return sourceFilePath === options.tasks.compileBrowserify.sourcePath;
});
out$.compileFile = compileFile = function(options, task, cb){
  fs.exists(task.sourcePath, function(exists){
    if (!exists) {
      info("| compile-browserify:skipping `" + task.sourcePath + "`.");
      return cb();
    }
    file.ensureFileDirectory(options, task, task.targetPath, function(error){
      ...
      bundle.on("bundle", function(bundleStream){
        ...
        writeStream.on("finish", function(){
          info("| compile-browserify:compile-file `" + task.sourcePath + "` > `" + task.targetPath + "`.");
          cb();
        });
