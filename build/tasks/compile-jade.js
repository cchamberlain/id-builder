...
var fs, jade, lsr, async, log, debug, info, warning, ref$, map, filter, file, sourceExtension, targetExtension, sourceFilePathMatches, compileChunk, compileFile, compileAllFiles, out$ = typeof exports != 'undefined' && exports || this;
fs = require('fs');
jade = require('jade');
lsr = require('lsr');
async = require('async');
...
out$.compileFile = compileFile = curry$(function(options, task, sourceFilePath, targetFilePath, cb){
  fs.readFile(sourceFilePath, function(error, chunk){
    ...
    compileChunk(options, task, chunk.toString(), sourceFilePath, function(error, compiledChunk){
      if (error) {
        return cb(error);
      }
      file.ensureFileDirectory(options, task, targetFilePath, function(error){
        ...
        fs.writeFile(targetFilePath, compiledChunk, function(error){
          ...
          info("| compile-jade:compile-file `" + sourceFilePath + "` > `" + targetFilePath + "`.");
          cb(null);
        });
      });
    });
