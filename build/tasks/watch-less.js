...
var idDebug, watch, less, dependencies, handlePath, handleAdd, handleAddDir, handleChange, handleUnlink, handleUnlinkDir, handleError, run, out$ = typeof exports != 'undefined' && exports || this;
...
handlePath = curry$(function(options, path, stat){
  ...
  targetPath = path.replace(options.sourcePath, options.targetPath).replace(RegExp('\\.' + less.sourceExtension + '$'), "." + less.targetExtension);
  less.compileFile(options, path, targetPath, function(error){
    if (error) {
      idDebug.error(error);
    }
  });
