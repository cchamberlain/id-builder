...
var fs, mkdirp, async, lsr, ref$, map, filter, compileBrowserify, compileCoffeescript, compileJade, compileLess, compileLivescript, compileStylus, file, sourceFilePathMatches, copyFile, copyAllFiles, out$ = typeof exports != 'undefined' && exports || this;
fs = require('fs');
...
out$.sourceFilePathMatches = sourceFilePathMatches = curry$(function(options, task, sourceFilePath){
  var answer;
  answer = !(compileBrowserify.sourceFilePathMatches(options, task, sourceFilePath) || compileCoffeescript.sourceFilePathMatches(options, task, sourceFilePath) || compileJade.sourceFilePathMatches(options, task, sourceFilePath) || compileLess.sourceFilePathMatches(options, task, sourceFilePath) || compileLivescript.sourceFilePathMatches(options, task, sourceFilePath) || compileStylus.sourceFilePathMatches(options, task, sourceFilePath));
  return answer;
});
  ...
  fs.readFile(sourceFilePath, function(error, readChunk){
    ...
    file.ensureFileDirectory(options, task, targetFilePath, function(error){
      ...
      fs.writeFile(targetFilePath, readChunk, function(error){
        ...
        console.log("| copy:copy-file `" + targetFilePath + "`.");
        cb(null);
      });
    });
  });
