...
var fs, LiveScript, lsr, async, log, debug, info, warning, ref$, map, filter, file, sourceExtension, targetExtension, sourceFilePathMatches, compileChunk, compileFile, compileAllFiles, out$ = typeof exports != 'undefined' && exports || this;
fs = require('fs');
...
out$.sourceFilePathMatches = sourceFilePathMatches = curry$(function(options, task, sourceFilePath){
  return sourceFilePath.match(RegExp('^' + options.tasks.compileLivescript.sourcePath + '.+\\.' + sourceExtension + '$'));
});
...
out$.compileFile = compileFile = curry$(function(options, task, sourceFilePath, targetFilePath, cb){
  fs.readFile(sourceFilePath, function(error, chunk){
    ...
    info("| compile-livescript:compile-file > `" + sourceFilePath + "`.");
    compileChunk(options, task, chunk.toString(), function(error, compiledChunk){
      ...
      file.ensureFileDirectory(options, task, targetFilePath, function(error){
        ...
        fs.writeFile(targetFilePath, compiledChunk, function(error){
          ...
          info("| compile-livescript:compile-file < `" + targetFilePath + "`.");
          cb(null);
        });
      });
    });
