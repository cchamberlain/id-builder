...
var fs, coffeeScript, lsr, async, LiveScript, ref$, map, filter, file, sourceExtension, targetExtension, sourceFilePathMatches, compileChunk, compileFile, compileAllFiles, out$ = typeof exports != 'undefined' && exports || this;
fs = require('fs');
    ...
    compileChunk(options, task, chunk.toString(), function(error, compiledChunk){
      ...
      file.ensureFileDirectory(options, task, targetFilePath, function(error){
        ...
        fs.writeFile(targetFilePath, compiledChunk, function(error){
          ...
          console.log("| livescript:compile-file `" + sourceFilePath + "` > `" + targetFilePath + "`.");
          cb(null);
        });
      });
    });
