...
var fs, less, lsr, async, ref$, map, filter, file, sourceExtension, targetExtension, sourceFilePathMatches, compileChunk, compileFile, compileAllFiles, out$ = typeof exports != 'undefined' && exports || this;
fs = require('fs');
less = require('less');
lsr = require('lsr');
async = require('async');
    ...
    compileChunk(options, task, chunk.toString(), function(error, compiledChunk){
      ...
      file.ensureFileDirectory(options, task, targetFilePath, function(error){
        ...
        fs.writeFile(targetFilePath, compiledChunk, function(error){
          ...
          console.log("| compile-less:compile-file `" + sourceFilePath + "` > `" + targetFilePath + "`.");
          cb(null);
        });
      });
    });
