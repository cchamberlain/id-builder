...
var fs, less, lsr, async, log, debug, info, warning, ref$, map, filter, file, sourceExtension, targetExtension, sourceFilePathMatches, compileChunk, compileFile, compileAllFiles, out$ = typeof exports != 'undefined' && exports || this;
fs = require('fs');
less = require('less');
lsr = require('lsr');
async = require('async');
...
out$.sourceFilePathMatches = sourceFilePathMatches = curry$(function(options, task, sourceFilePath){
  return sourceFilePath.match(RegExp('^' + options.tasks.compileLess.sourcePath + '.+\\.' + sourceExtension + '$'));
});
    ...
    compileChunk(options, task, chunk.toString(), function(error, compiledChunk){
      ...
      file.ensureFileDirectory(options, task, targetFilePath, function(error){
        ...
        fs.writeFile(targetFilePath, compiledChunk, function(error){
          ...
          info("| compile-less:compile-file `" + sourceFilePath + "` > `" + targetFilePath + "`.");
          cb(null);
        });
      });
    });
