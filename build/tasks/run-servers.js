...
var async, foreverMonitor, fs, idDebug, path, preludeLs, p, debug, info, warning, map, monitors, addPath, removePath, restartPath, sourceFilePathMatches, startServer, stopServer, restartServer, absolutePath, runServers, restartServers, out$ = typeof exports != 'undefined' && exports || this;
async = require('async');
...
out$.restartServer = restartServer = curry$(function(options, task, filePath, cb){
  var absolutePath;
  debug("run-servers restart-server", filePath);
  absolutePath = path.resolve(filePath);
  fs.exists(absolutePath, function(exists){
    if (!exists) {
...
absolutePath = curry$(function(task, path){
  return p.resolve(task.sourcePath + "/" + path);
});
out$.runServers = runServers = function(options, task, cb){
  var absolutePaths;
  absolutePaths = map(absolutePath(task), task.paths);
...
out$.restartServers = restartServers = function(options, task, cb){
  ...
  absolutePaths = res$;
  debug("run-servers restart-servers absolute-paths", absolutePaths);
